...
pub struct LengthExtensionForgery<const N: usize> {
    pub message: Vec<u8>,
    pub mac: [u8; N],
}
...
pub fn keyed_sha1_mac_length_extension_attack(
    ...
    message: &[u8],
    suffix_forgery: &[u8],
) -> Result<LengthExtensionForgery<20>, String> {
    // Try a range of key lengths until we find one that works.
    for key_len in 0..64 {
        // Pad the original message with "glue-padding", as that is what the
        // SHA1 algorithm would have done with the original message and we're
        // trying to get a SHA1 hasher with an equivalent state.
        let glue_padded_message = &md_pad(&[&b"0".repeat(key_len), message].concat())[key_len..];
        // Our message forgery will be the message with the glue-padding and
        // then our desired extension.
